...
// These pragma markers are supposed to help the DX when viewing in IDEs. Remove if not necessary
...
- (void)appendXML:(id)data toString:(NSMutableString *)xmlString withIndent:(NSString *)indent {
    if ([data isKindOfClass:[NSDictionary class]]) {
        for (NSString *key in data) {
            ...
            [self appendXML:data[key] toString:xmlString withIndent:[indent stringByAppendingString:@"  "]];
            [xmlString appendFormat:@"%@</%@>\n", indent, key];
        }
